...
func (r *BinaryReader) ReadArchive(ctx context.Context, path string) (loadiwad.RawArchive, error) {
	select {
	case <-ctx.Done():
		return loadiwad.RawArchive{}, ctx.Err()
	default:
	}

	fileData, err := os.ReadFile(path)
	if err != nil {
		return loadiwad.RawArchive{}, fmt.Errorf("read file %q: %w", path, err)
	}

	archive, err := parseBinary(fileData)
	if err != nil {
		return loadiwad.RawArchive{}, fmt.Errorf("parse %q: %w", path, err)
	}

	return archive, nil
}

func parseBinary(fileData []byte) (loadiwad.RawArchive, error) {
	if len(fileData) < headerSize {
		return loadiwad.RawArchive{}, ErrWADTooSmall
	}
	...
	if directoryEnd > uint64(len(fileData)) {
		return loadiwad.RawArchive{}, fmt.Errorf("%w: offset=%d size=%d file=%d", ErrInvalidDirectory, directoryOffset, directorySize, len(fileData))
	}

	lumps := make([]loadiwad.RawLump, 0, lumpCount)
	for i := uint32(0); i < lumpCount; i++ {
		...
		if entryEnd > uint64(len(fileData)) {
			return loadiwad.RawArchive{}, fmt.Errorf("%w at index %d", ErrDirectoryEntryBroken, i)
		}
		...
		if payloadEnd > uint64(len(fileData)) {
			return loadiwad.RawArchive{}, fmt.Errorf("%w: name=%s end=%d file=%d", ErrLumpOutOfBounds, lumpName, payloadEnd, len(fileData))
		}

		lumps = append(lumps, loadiwad.RawLump{
			Name:   lumpName,
			Offset: lumpOffset,
			Size:   lumpSize,
		})
