...
func (r *MapReader) ReadMap(ctx context.Context, wadPath string, mapName string) (loadmap.RawMap, error) {
	select {
	case <-ctx.Done():
		return loadmap.RawMap{}, ctx.Err()
	default:
	}

	fileData, err := os.ReadFile(wadPath)
	if err != nil {
		return loadmap.RawMap{}, fmt.Errorf("read file %q: %w", wadPath, err)
	}

	archive, err := parseBinary(fileData)
	if err != nil {
		return loadmap.RawMap{}, fmt.Errorf("parse archive %q: %w", wadPath, err)
	}

	startIdx := findMapStartIndex(archive.Lumps, mapName)
	if startIdx < 0 {
		return loadmap.RawMap{}, fmt.Errorf("%w: %s", ErrMapNotFound, mapName)
	}
	...
	if err != nil {
		return loadmap.RawMap{}, err
	}

	return rawMap, nil
}

func findMapStartIndex(lumps []loadiwad.RawLump, mapName string) int {
...
func buildRawMap(name string, lumpData map[string][]byte) (loadmap.RawMap, error) {
	things, ok := lumpData["THINGS"]
	if !ok {
		return loadmap.RawMap{}, ErrMissingThingsLump
	}
	linedefs, ok := lumpData["LINEDEFS"]
	if !ok {
		return loadmap.RawMap{}, ErrMissingLinedefsLump
	}
	sidedefs, ok := lumpData["SIDEDEFS"]
	if !ok {
		return loadmap.RawMap{}, ErrMissingSidedefsLump
	}
	vertexes, ok := lumpData["VERTEXES"]
	if !ok {
		return loadmap.RawMap{}, ErrMissingVertexesLump
	}
	sectors, ok := lumpData["SECTORS"]
	if !ok {
		return loadmap.RawMap{}, ErrMissingSectorsLump
	}

	return loadmap.RawMap{
		Name:     name,
		Things:   things,
		Linedefs: linedefs,
		Sidedefs: sidedefs,
